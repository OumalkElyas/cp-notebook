struct line {
    Pt<ll> p1,p2;
    line() {};
    line(const Pt<ll> &p1,const Pt<ll> &p2) : p1(p1),p2(p2) {};
    void read() { cin >> p1.x >> p1.y >> p2.x >> p2.y; }
    int intersect(const line &l) {
	ll cross[4];
	cross[0] = Pt<ll>::cross(p1,p2,l.p1),cross[1] = Pt<ll>::cross(p1,p2,l.p2);
	cross[2] = Pt<ll>::cross(l.p1,l.p2,p1),cross[3] = Pt<ll>::cross(l.p1,l.p2,p2);
...
private:
    bool belong(const line &l,const Pt<ll> &p) {
